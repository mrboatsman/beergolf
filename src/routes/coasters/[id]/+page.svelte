...
>
	...
	>
		<!-- Framsida -->
		<div
			class="-rotate-[0.6deg] rounded-[28px] border border-black/5 bg-card px-3 py-6 font-coaster text-print shadow-[0_10px_30px_-8px_rgba(60,50,30,0.35),0_2px_6px_rgba(60,50,30,0.15)] [backface-visibility:hidden] sm:px-10 sm:py-9"
		>
			<!-- Huvud: titel + vimpel -->
							...
							<tr class="border-b border-print/80">
								<td class="h-12 max-w-16 py-1 pr-1 align-top sm:max-w-none">
									<span class="block text-[9px] leading-none text-print/80"
										>Player {p.position}</span
									>
									<span class="font-hand text-lg leading-tight text-ink sm:hidden"
										>{shortName(p.name)}</span
									>
									<span class="hidden font-hand text-xl leading-tight text-ink sm:inline"
										>{p.name}</span
									>
									{#if !p.memberId}<span
										...
										>{/if}
									{#if p.signedAt}<span
											class="ml-1 align-middle text-xs text-print/70"
											title="Signerad">✓</span
										>{:else if amInvolved}
